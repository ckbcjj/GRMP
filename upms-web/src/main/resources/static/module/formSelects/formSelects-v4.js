...
(function (layui, window, factory) {
	...
	var v = '4.0.0.formSelects',
	    ...
	    quickBtns = [{ icon: 'iconfont icon-quanxuan', name: 'All', click: function click(id, cm) {
			cm.selectAll(id, true, true);
		} }, { icon: 'iconfont icon-qingkong', name: 'Clear', click: function click(id, cm) {
			cm.removeAll(id, true, true);
		} }, { icon: 'iconfont icon-fanxuan', name: 'Invert', click: function click(id, cm) {
			cm.reverse(id, true, true);
		} }, { icon: 'iconfont icon-pifu', name: 'Change Skin', click: function click(id, cm) {
			cm.skin(id);
		} }],
					...
					if (searchUrl) {
						...
						if (!ajaxConfig.beforeSearch || ajaxConfig.beforeSearch && ajaxConfig.beforeSearch instanceof Function && ajaxConfig.beforeSearch(id, searchUrl, inputValue)) {
							...
							fs.clearid = setTimeout(function () {
								...
								reElem.find('dd.' + FORM_NONE).addClass(FORM_EMPTY).text('waiting...');
						...
						if (!shows.length) {
							reElem.find('dd.' + FORM_NONE).addClass(FORM_EMPTY).text('no matches');
						} else {
	...
	Common.prototype.ajax = function (id, searchUrl, inputValue, isLinkage, linkageWidth, isSearch) {
		...
		$.ajax({
			...
			error: function error(err) {
				...
				reElem.find('dd.' + FORM_NONE).addClass(FORM_EMPTY).text('valor error');
	...
	Common.prototype.renderSelect = function (id, tips, select) {
		...
		arr.push('<dd class="' + FORM_SELECT_TIPS + ' ' + FORM_NONE + ' ' + (arr.length === 2 ? FORM_EMPTY : '') + '">no options</dd>');
		return arr.join('');
	};
